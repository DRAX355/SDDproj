...
import { 
  ...
  TrendingUp, GitMerge, ChevronRight
} from 'lucide-react';

export default function Home() {
  const [scrolled, setScrolled] = useState(false);
  const [activePerfTab, setActivePerfTab] = useState('voting');
  ...
  const perfDetails = [
    { id: 'voting', title: 'Soft Voting', icon: CheckCircle2, text: 'Mathematically averages the exact confidence percentages from all three neural networks. This continuous consensus ensures the most probabilistically sound diagnosis possible rather than simple counting.' },
    { id: 'diverse', title: 'Diverse Learning', icon: BrainCircuit, text: 'Each model learns different feature sets. EfficientNet excels at texture identification, while ResNet captures broad shapes and structural anomalies perfectly.' },
    { id: 'error', title: 'Error Mitigation', icon: AlertTriangle, text: 'If one model miscalculates due to poor lighting or blur, the other two models outvote it to maintain high accuracy and drastically reduce false positives.' }
  ];
    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col selection:bg-emerald-500 selection:text-white">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 py-4 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-lg py-3' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <Link to="/" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center gap-2 focus:outline-none group">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Process', 'Technology', 'Performance', 'Capabilities'].map((item) => (
              <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className={`text-sm font-semibold relative group transition-colors ${scrolled ? 'text-gray-600 hover:text-emerald-600' : 'text-emerald-50 hover:text-white'}`}>
                {item}
            ...
            <button onClick={() => scrollToSection('portals')} className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 transform hover:-translate-y-0.5 hover:shadow-lg ${scrolled ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/20' : 'bg-white text-emerald-900 hover:bg-emerald-50 shadow-black/10'}`}>
              Login Portal
            </button>
          </div>
        </div>
      ...
      <div id="home" className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 text-white relative flex items-center justify-center min-h-screen pt-20 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10 -mt-16 md:-mt-24">
          ...
          <button onClick={() => scrollToSection('portals')} className="group relative px-10 py-5 bg-white text-emerald-900 rounded-full font-bold text-lg shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(255,255,255,0.5)] hover:-translate-y-1 active:scale-95 transition-all duration-300 overflow-hidden">
            ...
            <span className="relative flex items-center gap-3">Start Diagnosis <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></span>
          </button>
        </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {['EfficientNet', 'ResNet50', 'MobileNet'].map(m => (
                        <div key={m} className="bg-slate-700/50 h-16 rounded-xl flex items-center justify-center text-[10px] md:text-xs font-mono text-gray-300 border border-slate-600 hover:border-emerald-500/50 hover:bg-slate-700 transition-all duration-300 cursor-default">{m}</div>
                    ))}
                  </div>
      ...
      {/* --- COMPACT INTERACTIVE PERFORMANCE SECTION --- */}
      <div id="performance" className="py-16 bg-white relative z-10 border-b border-slate-100 scroll-mt-16">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-10">
            <span className="text-emerald-600 font-bold tracking-wider uppercase text-sm mb-2 block">Metrics</span>
            <h2 className="text-3xl font-extrabold text-gray-900">Ensemble Superiority</h2>
          </div>

          <div className="flex flex-col md:flex-row gap-8 items-stretch">
            
            {/* Graph Left */}
            <div className="w-full md:w-1/2 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden flex flex-col justify-center">
              <div className="absolute top-0 right-0 w-48 h-48 bg-emerald-50 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/4 pointer-events-none"></div>
              <h3 className="font-bold text-xl mb-6 text-gray-800 flex items-center gap-2 relative z-10">
                <TrendingUp className="text-emerald-500 h-5 w-5" /> Validation Accuracy
              </h3>
              
              <div className="space-y-6 relative z-10">
                <div className="group">
                  <div className="flex justify-between text-xs mb-1.5 font-bold text-gray-500"><span>MobileNetV2</span><span>85.2%</span></div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5"><div className="bg-slate-300 h-2.5 rounded-full" style={{ width: '85.2%' }}></div></div>
                </div>
                <div className="group">
                  <div className="flex justify-between text-xs mb-1.5 font-bold text-gray-500"><span>ResNet50</span><span>87.8%</span></div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5"><div className="bg-slate-300 h-2.5 rounded-full" style={{ width: '87.8%' }}></div></div>
                </div>
                <div className="group">
                  <div className="flex justify-between text-xs mb-1.5 font-bold text-gray-500"><span>EfficientNetB0</span><span>89.4%</span></div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5"><div className="bg-slate-300 h-2.5 rounded-full" style={{ width: '89.4%' }}></div></div>
                </div>
                <div className="pt-4 mt-2 border-t border-slate-100">
                  <div className="flex justify-between text-sm mb-2 font-black text-emerald-700"><span className="flex items-center gap-1.5"><GitMerge size={16}/> Ensemble</span><span className="text-base">94.6%</span></div>
                  <div className="w-full bg-emerald-100 rounded-full h-3 shadow-inner"><div className="bg-gradient-to-r from-emerald-400 to-teal-500 h-3 rounded-full relative" style={{ width: '94.6%' }}><div className="absolute inset-0 bg-white/20 animate-pulse rounded-full"></div></div></div>
                </div>
              </div>
            </div>
              ...
              <div className="flex bg-slate-100 p-1 rounded-2xl mb-4">
                {perfDetails.map(tab => (
                  <button 
                    ...
                    className={`flex-1 py-2.5 text-xs font-bold rounded-xl transition-all ${activePerfTab === tab.id ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    {tab.title}
                  </button>
                ))}
              </div>
              <div className="bg-emerald-50/50 flex-1 rounded-3xl border border-emerald-100 p-8 flex flex-col justify-center animate-in fade-in transition-all">
                {perfDetails.map(tab => (
                   activePerfTab === tab.id && (
                     <div key={tab.id} className="animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-white w-12 h-12 rounded-xl flex items-center justify-center text-emerald-600 mb-5 shadow-sm border border-emerald-100">
                          <tab.icon size={24} />
                        </div>
                        <h4 className="font-bold text-gray-900 text-xl mb-3">{tab.title}</h4>
                        <p className="text-gray-600 text-sm leading-relaxed">{tab.text}</p>
                     </div>
                   )
                ))}
              </div>
      ...
      {/* --- SPACIOUS CAPABILITIES --- */}
      <div id="capabilities" className="py-24 bg-slate-50 border-b border-slate-200 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-emerald-600 font-bold uppercase text-sm mb-2 block">Scope</span>
      ...
      <div id="portals" className="py-24 bg-white scroll-mt-20">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <Link to="/auth?role=patient" className="group bg-slate-50 p-10 rounded-[2rem] shadow-sm border border-slate-100 hover:bg-white hover:shadow-2xl hover:shadow-emerald-500/10 hover:-translate-y-2 hover:border-emerald-100 transition-all duration-300 relative overflow-hidden flex flex-col items-center">
                ...
                <div className="bg-white w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-emerald-600 shadow-sm border border-emerald-50 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500"><User size={40}/></div>
            ...
            <Link to="/auth?role=student" className="group bg-slate-50 p-10 rounded-[2rem] shadow-sm border border-slate-100 hover:bg-white hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-2 hover:border-blue-100 transition-all duration-300 relative overflow-hidden flex flex-col items-center">
                ...
                <div className="bg-white w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-blue-600 shadow-sm border border-blue-50 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500"><BookOpen size={40}/></div>
            ...
            <Link to="/auth?role=admin" className="group bg-slate-50 p-10 rounded-[2rem] shadow-sm border border-slate-100 hover:bg-white hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-2 hover:border-purple-100 transition-all duration-300 relative overflow-hidden flex flex-col items-center">
                ...
                <div className="bg-white w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-purple-600 shadow-sm border border-purple-50 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500"><ShieldCheck size={40}/></div>
